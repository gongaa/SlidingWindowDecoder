...
)
{
  ...
  for (i=0; i<mod2sparse_rows(m); i++) {
    ...
    while (!mod2sparse_at_end(e)) {
      ...
      f->check_to_bit = e->check_to_bit;
      f->bit_to_check = e->bit_to_check;
...
)
{
  ...
  for (j = 0; j < mod2sparse_cols(r); j++) {
    ...
    // fprintf(stderr, "inserting column %d of m \n", cols[j]);
    e = mod2sparse_first_in_col(m, cols[j]);
    while (!mod2sparse_at_end(e)) {
      // fprintf(stderr, "row id = %d, insert to j=%d\n", e->row, j);
      mod2sparse_insert(r, e->row, j);
      // fprintf(stderr, "after insert to j=%d\n", j);
...
)
{
  ...
  if (mod2sparse_at_end(re) || mod2sparse_col(re)<col) return 0;
  if (mod2sparse_col(re)==col) return re;

  ce = mod2sparse_last_in_col(m, col);
  if (mod2sparse_at_end(ce) || mod2sparse_row(ce)<row) return 0;
  if (mod2sparse_row(ce)==row) return ce;
...
)
{
  ...
  if (!mod2sparse_at_end(re) && mod2sparse_col(re)==col) return re;

  if (mod2sparse_at_end(re) || mod2sparse_col(re)<col) re = re->right;
  else {
    re = mod2sparse_first_in_row(m, row);
    for (;;) {
      if (!mod2sparse_at_end(re) && mod2sparse_col(re)==col) return re;

      if (mod2sparse_at_end(re) || mod2sparse_col(re)>col) break;
  ...
  if (mod2sparse_at_end(ce) || mod2sparse_row(ce)<row) ce = ce->down;
  else {
    ce = mod2sparse_first_in_col(m, col);
    for (;;) {
      if (!mod2sparse_at_end(ce) && mod2sparse_row(ce)==row) {
...
)
{
  ...
  for (i = 0; i<mod2sparse_rows(m1); i++) {
    ...
    if (!mod2sparse_at_end(e1) || !mod2sparse_at_end(e2)) return 0;
  }
...
)
{
  ...
  for (i = 0; i<mod2sparse_rows(m1); i++) {
    if (mod2sparse_at_end(mod2sparse_first_in_row(m1,i))) continue;
