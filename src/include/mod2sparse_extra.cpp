...
char* decimal_to_binary_reverse(int n, int k) { // memory leak!!
    char* binary_number;
    int divisor;
    int remainder;
    divisor = n;
    binary_number = (char*) chk_alloc(k, sizeof(char *));
    for(int i=0; i<k; i++) {
        remainder = divisor%2;
...
int mod2sparse_rank(mod2sparse *A) {
    ...
    cols = (int*) chk_alloc(N, sizeof(*rows));
    rows = (int*) chk_alloc(M, sizeof(*cols));

    int abandon_number = 0;  	/* Number of columns to abandon at some point *//* When to abandon these columns */
    int abandon_when = 0;

    int submatrix_size = (M>N) ? N : M; // min(M, N)
...
char *x) {
    ...
    N = mod2sparse_cols(U);
    R = mod2sparse_cols(L);
    forward_b = (char*) chk_alloc(R, sizeof(*forward_b));
...
int mod2sparse_decomp_osd(
    ...
    int abandon_number = 0;  	/* Number of columns to abandon at some point */
    int abandon_when = 0;       /* When to abandon these columns */
    mod2sparse_strategy strategy = Mod2sparse_first;/* Strategy to follow in picking rows/columns */
